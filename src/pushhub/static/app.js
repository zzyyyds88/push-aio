...
const API_KEY_STORAGE = "pushhub_api_key";
// 兼容旧版本 localStorage key（push-aio 时代），首次加载时迁移过来
(function migrateLegacyKey() {
  const legacy = localStorage.getItem("push_aio_api_key");
  if (legacy && !localStorage.getItem(API_KEY_STORAGE)) {
    localStorage.setItem(API_KEY_STORAGE, legacy);
  }
  if (legacy) localStorage.removeItem("push_aio_api_key");
})();
...
function renderLoginView(setupMode) {
  ...
  if (setupMode) {
    ...
    title.textContent = "PushHub";
    lead.textContent = "按渠道类型调度：同类型层级（主推送 → 备用1 → 备用2 ...）逐个尝试，全失败升级全局紧急层级（并发发送）。";
...
function navigate(route) {
  ...
  if (route === "add-channel") renderChannelForm();
...
function renderDashboard() {
  ...
  // 渠道状态概览：必须用 state.status.channels（含 online/label/detail 等状态字段），
  // 不能用 state.channels（仅 ChannelOut，无 online 字段，会导致所有启用渠道被误判为"异常"）
  const channelsBox = document.getElementById("dashboard-channels");
  const statusChannels = (state.status && state.status.channels) || [];
  if (!statusChannels.length) {
    channelsBox.innerHTML = `<div class="list-item"><span class="meta">还没有渠道，前往「渠道管理」新增一个。</span></div>`;
  } else {
    // 与渠道管理页排序保持一致：非紧急渠道按 priority 升序（主推送 → 备用1 → 备用2 …），紧急渠道置后
    const sorted = [...statusChannels].sort((a, b) => {
      if (a.is_emergency !== b.is_emergency) return a.is_emergency ? 1 : -1;
      return (a.priority - b.priority) || (a.id - b.id);
    });
    channelsBox.innerHTML = sorted.map((item) => {
...
function renderChannelsPage() {
  ...
  if (sortedTypes.length) {
    html += `<div class="section-title">按类型调度</div>`;
...
function renderMainRow(item, idx, total) {
  ...
  const roleTag = idx === 0
    ...
    : `<span class="tag tag-backup">备用${cnNums[idx - 1] || idx}</span>`;
  ...
  return `
    <div class="channel-row ${expanded ? "expanded" : ""}" data-id="${item.id}" data-type="${escapeHtml(item.type)}" draggable="true">
      <div class="channel-row-head">
        ...
        <div class="ctags">${roleTag}<span class="tag tag-muted">${escapeHtml(label)}</span></div>
...
function renderChannelForm() {
  ...
  form.innerHTML = `
    ...
    <p class="meta">建议先点「先测试一下」确认配置可用再保存。新建渠道默认加入同类型层级末尾。需要作为兜底紧急渠道时，到「渠道管理」页用"改为紧急通道"按钮切换。</p>
  `;

  // 注意：submit 监听器在 DOMContentLoaded 里只绑一次，避免每次打开抽屉重复绑定导致多次提交
...
async function handleChannelAction(event) {
  ...
  if (action === "add-type") {
    // 分组内「添加」按钮：跳转到新增渠道页（独立页面，不再是抽屉）
    navigate("add-channel");
    // 预选渠道类型：等 renderChannelForm 渲染完后设置 select 值
    const preselectType = button.dataset.type;
    if (preselectType) {
      requestAnimationFrame(() => {
        const typeSelect = document.querySelector('#channel-form select[name="type"]');
        if (typeSelect) {
...
function renderNotifyResult(result) {
  ...
  if (result.escalated) {
    escalationBanner = `<div class="banner banner-warn">⚠ 同类型层级全部失败，已升级到全局紧急层级（并发发送所有紧急渠道）</div>`;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 新增渠道表单
  // submit 监听器只绑一次，避免重复绑定导致多次提交（重复创建渠道）
